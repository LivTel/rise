...
*/
/* fits_flip_double.c
** $Header: /space/home/eng/cjm/cvs/rise/ccd/misc/fits_flip_double.c,v 1.1 2009-10-15 10:15:44 cjm Exp $
*/
/**
 * <pre>
 * fits_flip_double -i &lt;Input FITS filename&gt; -o &lt;Output FITS filename&gt; [-x] [-y]
 * </pre>
 * fits_flip_double flips the image data around one or more axes.
 * The input file should have BITPIX -32 (float). The output file is written in this format.
 ...
 */
static char rcsid[] = "$Id: fits_flip_double.c,v 1.1 2009-10-15 10:15:44 cjm Exp $";
/**
...
/* open input files */
	...
	if(retval)
	{
		...
		fprintf(stderr,"fits_flip_double: Open %s failed.\n",Input_Filename);
		Close();
		return FALSE;
	}
	return TRUE;
...
/* open output files */
	...
	{
		fits_report_error(stderr,status);
		fprintf(stderr,"fits_flip_double: Create %s for output failed.\n",Output_Filename);
		return FALSE;
	}
/* copy the input files (0) header to the output */
	...
	{
		fits_report_error(stderr,status);
		fprintf(stderr,"fits_flip_double: Copy header to %s failed.\n",Output_Filename);
		return FALSE;
	}
...
static int Get_Axes(void)
{
	...
	{
		fprintf(stderr,"fits_flip_double: Wrong NAXIS value(%d).\n",integer_value);
		return FALSE;
	}
	/* get naxis1,naxis2 */
...
/* initialse pointers to NULL */
	Fits_Data = NULL;
/* allocate */
	Fits_Data = (float *)malloc(Fits_Axis_List[0]*Fits_Axis_List[1]*sizeof(float));
	if(Fits_Data == NULL)
	{
		fprintf(stderr,"fits_flip_double: failed to allocate memory (%d,%d).\n",
...
static int Read_Data(void)
{
	...
	retval = fits_read_img(Input_Fits_Fp,TFLOAT,1,Fits_Axis_List[0]*Fits_Axis_List[1],NULL,
...
static int Write_Data(void)
{
	...
	retval = fits_write_img(Output_Fits_Fp,TFLOAT,1,Fits_Axis_List[0]*Fits_Axis_List[1],
		Fits_Data,&status);
	if(retval)
	{
		fits_report_error(stderr,status);
...
static void Help(void)
{
	...
	fprintf(stdout,"fits_flip_double -i[nput] <FITS filename> -o[utput] <FITS filename> "
			"[-x][-y][-help]\n");
}

/*
** $Log: not supported by cvs2svn $
** Revision 1.3  2006/05/16 18:22:43  cjm
** gnuify: Added GNU General Public License.
**
** Revision 1.2  2003/02/19 12:42:01  cjm
** Fixed Flip X routine, it was flipping the top (y) half of the image twice!
**
** Revision 1.1  2003/01/20 19:27:34  cjm
** Initial revision
**
